...
[https://github.com/jjerisha/Jerisha_Josh_201_A06](https://github.com/jjerisha/Jerisha_Josh_201_A06)
...
```{r}
lebron_pts1 <- player_info[[2]][[4]] # extract via position indexing
print(lebron_pts1)

lebron_pts2 <- player_info$LJ$points # extract via names
print(lebron_pts2)
...
```{r}
player_heights <- c(78, 81, 78, 86, 85) # element positions correspond to player positions in player_info (e.g. player_heights[5] = 85)

for(i in 1:length(player_info)){ # position indexing
  player_info[[i]]$height <- player_heights[i]
}
...
# initialize user-defined function to be applied to every element of player_info
ppg_func <- function(player){
  player$points / player$games_played
}

points_per_game <- lapply(player_info, ppg_func) # output list
...
```{r}
points_per_game_rounded <- sapply(points_per_game, round, 2) # output vector

print(points_per_game_rounded)
...
# NOTE: The 'round' function rounds odd numbers ending in '5' up and even numbers ending in '5' down.

mat_round <- function(matrix){ # input is matrix of whole numbers
  for(i in 1:length(matrix)){
    matrix[i] <- round(matrix[i], -(nchar(matrix[i]) - 1)) # number of rounded digits is negative because rounding positions are left of the decimal point
  }
  return(matrix)
}

mat_rand_rounded <- apply(mat_rand, MARGIN = 1:2, mat_round)
...
```{r}
months_cols <- max_temps[, 3:14]
avg_temps <- colMeans(months_cols)
print(avg_temps)
...
```{r}
max_temps_CO <- max_temps[max_temps[1] == 'CO',]
max_temps_OH <- max_temps[max_temps[1] == 'OH',]
max_temps_VA <- max_temps[max_temps[1] == 'VA',]
...
```{r}
# calculate median for each year and store as list with elements named by year ('annual_medians')
median_values <- apply(max_temps_CO[, 3:14], 1, median)
year_names <- as.character(unlist(max_temps_CO[, 2]))
annual_medians <- as.list(median_values)
names(annual_medians) <- year_names

max_median_value <- max(as.numeric(annual_medians))
max_median_year <- names(annual_medians)[which.max(as.numeric(annual_medians))]
min_median_value <- min(as.numeric(annual_medians))
min_median_year <- names(annual_medians)[which.min(as.numeric(annual_medians))]

# print output statement
print(paste0('The maximum median annual temperature of ',
             max_median_value, 
             ' occurred in ',
             max_median_year,
             ' and the minimum median annual temperature of ',
             min_median_value, 
             ' occurred in ',
             min_median_year, 
             '.'))
...
digit_sum_output <- digit_sum(935068)
print(digit_sum_output)
